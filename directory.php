<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<title>Harvard Medical Intranet Directory</title>
	<link rel="stylesheet" href="css/style.css">
</head>
<body>
	<header class="cd-main-header">
		<a href="#0" class="cd-logo"><img width="140px" src="img/cd-logo.svg" alt="Logo"></a>
		
		<div class="cd-search is-hidden">
			<form action="/app/search.html">
				<input type="search" placeholder="Search...">
			</form>
		</div> <!-- cd-search -->

		<a href="#0" class="cd-nav-trigger">Menu<span></span></a>

		<nav class="cd-nav">
			<ul class="cd-top-nav">
				<li><a href="#0">Tour</a></li>
				<li><a href="#0">Support</a></li>
				<li class="has-children account">
					<a href="#0">
						<img src="img/cd-avatar.png" alt="avatar">
						Account
					</a>

					<ul>
						<li><a href="#0">My Account</a></li>
						<li><a href="#0">Edit Account</a></li>
						<li><a href="#0">Logout</a></li>
					</ul>
				</li>
			</ul>
		</nav>
	</header> <!-- .cd-main-header -->

	<main class="cd-main-content">
		<nav class="cd-side-nav">
			<ul>
				<li class="cd-label">Main</li>
				<li class="has-children active overview">
					<a href="#0">Overview</a>
					
					<ul>
						<li><a href="#0">All Data</a></li>
						<li><a href="#0">Quick Link 1</a></li>
						<li><a href="#0">Quick Link 2</a></li>
						<li><a href="#0">Quick Link 3</a></li>
					</ul>
				</li>
				<li class="has-children notifications ">
					<a href="#0">Notifications<span class="count">4</span></a>
					
					<ul>
						<li><a href="#0">All Notifications</a></li>
						<li><a href="#0">Department</a></li>
						<li><a href="#0">Admin</a></li>
						<li><a href="#0">Courses</a></li>
					</ul>
				</li>

				<li class="has-children comments">
					<a href="#0">Content</a>
					
					<ul>
						<li><a href="#0">All Content</a></li>
						<li><a href="#0">Edit Content</a></li>
					</ul>
				</li>
			</ul>

			<ul>
				<li class="cd-label">Secondary</li>
				<li class="has-children bookmarks">
					<a href="#0">Bookmarks</a>
					
					<ul>
						<li><a href="#0">All Bookmarks</a></li>
						<li><a href="#0">Edit Bookmark</a></li>
						<li><a href="#0">Import Bookmark</a></li>
					</ul>
				</li>
				<li class="has-children images">
					<a href="#0">XXXX</a>
					
					<ul>
						<li><a href="#0">XXXXX</a></li>
						<li><a href="#0">XXXXX</a></li>
					</ul>
				</li>

				<li class="has-children users">
					<a href="#0">Admin</a>
					
					<ul>
						<li><a href="#0">All Users</a></li>
						<li><a href="#0">Edit User</a></li>
						<li><a href="#0">Add User</a></li>
					</ul>
				</li>
			</ul>

			<ul>
				<li class="cd-label">Action</li>
				<li class="action-btn"><a href="#0">+ Button</a></li>
			</ul>
		</nav>

		<div class="content-wrapper">
			<section class="row-module">
				<div class="full-module">
					<div class="cal-module">
						<h1>Upcoming Events</h1>
						<p>You have 4 events coming up.</p>
						<ul class="cal-list">
							<li class="cal-item">
								<span class="cal-time">9:00 a.m. – 10:30 a.m.</span>
								<span class="cal-event-title">Department Meeting</span>
								<ul class="cal-detail">
									<li><span class="cal-detail-label">Location</span>HMS Conference</li>
									<li><span class="cal-detail-label">Dial-in</span>617-503-0002</li>
									<li><span class="cal-detail-label">Room</span>4031</li>
									<li><span class="cal-detail-label">Pin</span>523</li>
									<li><span class="cal-detail-label">Attendees</span>Jim Gould, Julie Huang, Rick Shea, Jeff Diciaccio</li>
								</ul>
							</li>
							<li class="cal-item">
								<span class="cal-time">11:00 a.m. – 1:00 p.m.</span>
								<span class="cal-event-title">Budget Group</span>
								<ul class="cal-detail">
									<li><span class="cal-detail-label">Location</span>HMS Conference</li>
									<li><span class="cal-detail-label">Dial-in</span>617-503-0002</li>
									<li><span class="cal-detail-label">Room</span>4031</li>
									<li><span class="cal-detail-label">Pin</span>523</li>
									<li><span class="cal-detail-label">Attendees</span>Jim Gould, Julie Huang, Rick Shea, Jeff Diciaccio</li>
								</ul>
							</li>
							<li class="cal-item">
								<span class="cal-time">1:00 p.m.</span>
								<span class="cal-event-title">Bio Group</span>
								<ul class="cal-detail">
									<li><span class="cal-detail-label">Location</span>HMS Conference</li>
								</ul>
							</li>								
						</ul>
						<button href="#">See More</button>
					</div>
				</div><!-- .full-module -->
			</section>
			<section class="row-module">
			<div class="full-module">
				<article class="type-system-traditional">
					<p class="type">Article Type</p>
					<h1>Article Heading</h1>
					<h2>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Dolor, perferendis! Odit quisquam <code>code block</code> dicta illum ipsa quod natus iure aliquid necessitatibus veritatis, itaque magnam aliquam, dolorum nostrum nemo, explicabo perspiciatis nobis!</h2>
					<p class="date">30 Mar 2014</p>
					<p><span>Lorem ipsum dolor sit amet</span>, consectetur adipisicing elit. Consequatur a, ullam, voluptatum incidunt neque doloremque vel inventore distinctio laudantium harum</a> illo quam nulla dolor alias iure impedit! Accusamus! Lorem ipsum dolor sit amet, consectetur adipisicing elit. Accusamus! Lorem ipsum dolor sit amet, consectetur adipisicing elit. Consequatur, a, ullam, voluptatum incidunt neque porro numquam doloremque vel inventore distinctio laudantium harum illo quam nulla dolor alias iure impedit.
					<a href="javascript:void(0)" class="read-more">Read More <span>&rsaquo;</span></a>
					</p>
					<h3>Subheading lorem</h3>
					<p><span>Consequatur ullam, voluptatum</span> incidunt neque porro numquam doloremque vel inventore distinctio laudantium harum illo quam nulla dolor alias iure impedit. Accusamus. Consequatur, a, ullam, voluptatum incidunt neque porro numquam doloremque vel inventore distinctio laudantium harum illo quam nulla dolor alias iure impedit! Accusamus.</p>
					<hr>
					<p class="author">Author Name</p>
				</article>
			</div>
			</section>
			<section class="row-module">
				<div class="half-module">
					<h1>Calendar</h1>
					<p>Today at a glance.</p>
					<ul>
						<li>Department Meeting</li>
						<li>Budget Group</li>
						<li>Monthly Rounds</li>
						<li>Reporting</li>
						<li>Group Survey Meeting</li>
						<li>Budget Group</li>
						<li>Department Meeting</li>
						<li><em>See More</em></li>
					</ul>
				</div><!-- .full-module -->
				<div class="half-module">
					<h1>Calendar</h1>
					<p>Today at a glance.</p>
					<ul>
						<li>Department Meeting</li>
						<li>Budget Group</li>
						<li>Monthly Rounds</li>
						<li>Reporting</li>
						<li>Group Survey Meeting</li>
						<li>Budget Group</li>
						<li>Department Meeting</li>
						<li><em>See More</em></li>
					</ul>
				</div><!-- .full-module -->
			</section>
		</div> <!-- .content-wrapper -->
	</main> <!-- .cd-main-content -->
<script src="js/jquery-2.1.4.js"></script>
<script src="js/jquery.menu-aim.js"></script>
<script src="js/main.js"></script> <!-- Resource jQuery -->
</body>
</html>